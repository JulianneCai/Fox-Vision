...
from utils.transforms import Rescale, ToTensor, RandomHorizontalFlip
...
class FoxDB:
    ...
    def _adapt_array(self, arr) -> None:
    ...
    def _convert_array(self, text) -> np.ndarray:
    ...
    def create_fox_train(self) -> None:
    ...
    def insert_fox_train(self, verbose=False) -> None:
    ...
    def retrieve_matrix(self, idx) -> np.ndarray:
    ...
    def retrieve_matrix_by_class(self, idx, class_name) -> np.ndarray:
        """Retrives the idx'th RGB matrix in the SQL database that belongs 
        to class class_name
        ...
        Raises:
            ValueError: if class_name is not a valid class
    ...
    def retrieve_class_id(self, idx) -> np.ndarray:
    ...
    def retrieve_class_id_by_class(self, idx, class_name) -> np.ndarray:
        ...
        Returns:
            np.ndarray: the idx'th row of class class_name
    ...
    def to_tensor(
        ...
        """Converts input and label to torch.Tensor objects so that it can be 
        fed into a CNN

        Args:
            input (np.ndarray): the RGB matrix of the image 
            label (int): the label-encoded name of the class

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: tuple of input, label, but converted
            to torch.Tensor object
        """
